...
<div>
   <div class="aui-tab" id="tab">
    ...
    <ul class="aui-list aui-media-list" v-if="isActive<4">
            <li class="aui-list-item" v-for="updatas in updata">
                <div class="aui-media-list-item-inner">
                    <div class="aui-list-item-inner">
                        <div class="aui-list-item-text aui-text-left">
                            <div class="aui-list-item-title aui-font-size-18">{{updatas.tit}}</div>
                        </div>
                        <div class="aui-row aui-row-padded aui-margin-t-5" v-if="updatas.img.length > 1">
                        <div class="aui-col-xs-4" v-for="upimg in updatas.img">
                            <img :src="upimg"/>
                        </div>
                    </div>
                        <div class="aui-info aui-padded-b-0">
                            <div class="aui-info-item aui-font-size-12">
                                <span class="aui-margin-l-5">{{updatas.source}}{{updatas.comlist}}评论</span>
                                </div>
                        </div>
                    </div>
                    <div class="aui-list-item-media" v-if="updatas.img.length < 2">
                        <img v-for="upimg in updatas.img" :src="upimg">
                    </div>
                </div>
            </li>
      </ul>
      <ul class="padd-bot" v-if="isActive>=4">
...
<script>
  export default {
    ...
    created(){
    let _this = this;
    this.$ajax.get('/api/home').then(function (response) {
